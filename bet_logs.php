...
// এপিআই গেমের বাজির ইতিহাস নিয়ে আসা
$bet_query = "SELECT * FROM bets WHERE username = '$u' ORDER BY id DESC LIMIT 50";
...
<div style="padding: 20px; color: white; font-family: sans-serif; min-height: 80vh;">
    ...
    <?php if($bet_res && $bet_res->num_rows > 0): while($row = $bet_res->fetch_assoc()): ?>
        <div style="background: #111; border: 1px solid #222; padding: 15px; border-radius: 12px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:5px;">
                    <span style="background:#333; color:#fff; font-size:9px; padding:2px 6px; border-radius:4px;"><?php echo $row['game_id']; ?></span>
                    <small style="color:#666; font-size:10px;"><?php echo date('d M, h:i A', strtotime($row['date'])); ?></small>
                </div>
            ...
            <div style="text-align: right;">
                <?php if($row['status'] == 'win'): ?>
                    <span style="color:#00ff88; font-weight:bold; font-size:14px;">+ ৳ <?php echo number_format($row['amount'] * 1.9, 2); ?></span>
    ...
    <?php endwhile; else: ?>
        <div style="text-align:center; margin-top:100px; color:#555;">
            ...
            <a href="index.php" style="color:#00ff88; text-decoration:none;">গেম খেলা শুরু করুন →</a>
        </div>
    <?php endif; ?>
</div>
