...
// ১. সেটেলমেন্ট বাটন লজিক
if(isset($_POST['settle_now'])){
    $aff_id = $_POST['aff_id'];
    $amount = (float)$_POST['settle_amount'];
    
    // aff_balance এ টাকা যোগ করা এবং লস/প্রফিট রিসেট করা
...
while($row = $affiliates->fetch_assoc()){
    $net = (float)($row['player_loss_profit'] ?? 0); 
    $share = $net * 0.47; 
    
    echo "<tr>";
    echo "<td>".$row['username']."</td>";
    echo "<td>৳".number_format($net, 2)."</td>";
    echo "<td style='color:#ffdf1b;'>৳".number_format($share, 2)."</td>";
    echo "<td>
            <form method='POST'>
                <input type='hidden' name='aff_id' value='".$row['id']."'>
                <input type='hidden' name='settle_amount' value='".$share."'>
                <button type='submit' name='settle_now' style='background:#00ff88; border:none; padding:8px; border-radius:5px; cursor:pointer;'>Settle 47%</button>
            </form>
          </td>";
    echo "</tr>";
}
